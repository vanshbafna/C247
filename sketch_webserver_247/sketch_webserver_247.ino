...
const char* ssid = "The Bafna's Home";
const char* password = "Bhkne321@";
...
void setup(void) {
  ...
  if(MDNS.begin("esp32")){
    Serial.println("MDNS responderStarted");
    
    }

   server.on("/", handleRoot);
   server.on("/roomLight/on", []() { server.send(200, "text/html",button); digitalWrite(LED_1,HIGH);  });
   server.on("/roomLight/off", []() { server.send(200, "text/html",button); digitalWrite(LED_1,LOW);  });
   server.on("/roomBell/on", []() { server.send(200, "text/html",button); digitalWrite(BUZZER,HIGH);  });
   server.on("/roomBell/off", []() { server.send(200, "text/html",button); digitalWrite(BUZZER,LOW);  });
